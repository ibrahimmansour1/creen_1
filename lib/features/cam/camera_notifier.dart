...
class CameraNotifier extends ChangeNotifier {
  ...
  void changeCamera([int? specificIndex]) {
    // print("status==> ${status is CameraStatusEmpty}");
    if (status is CameraStatusSuccess) {
      ...
      // print("status.preview.cameras ==> ${status.preview.cameras}");
      final index = status.preview.indexSelected;
      // print("status.preview.indexSelected ==> ${status.preview.indexSelected}");
